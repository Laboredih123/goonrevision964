...
/obj/item/weapon/gun/energy/taser_gun/attack(mob/M as mob, mob/carbon/user as mob)
	...
	if(src.charges >= 1)
		if (user.intent == "hurt")
			H.show_viewers("\red <B>[H] has been knocked unconscious with the taser gun by [user]!</B>")
			if (prob(20))
				H.knockout_until(5)
...
/obj/d_girders/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/sheet/metal))
		...
		if (W:amount < 1)
			del(W)
			return
		var/turf/station/r_wall/false_rwall/fwall = new /turf/station/r_wall/false_rwall( src.loc )
		fwall.known_by += user
		user << "False rwall constructed."
		W:amount--
		if (W:amount < 1)
			del(W)
		del(src)
