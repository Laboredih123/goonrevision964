/datum/admin_power/private_message
	...
	get_desc(mob/M)
		return "<A href='?src=\ref[M];priv_msg=1'>PM</A>"

/client/proc/private_message(mob/M as mob)
	var/t = text_input("Message:", "Private message to [M.key]")  as text
	if(!t)
		return
	M << "\blue PM from-<B><A href='?src=\ref[usr];priv_msg=1'>[usr.key]</A></B>: [t]"
	usr << "\blue PM to-<B><A href='?src=\ref[M];priv_msg=1'>[M.key]</A></B>: [t]"
