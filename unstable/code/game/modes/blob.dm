...
/datum/game_mode/blob/proc/life()
	if(!blobs.len) return
	for(var/i = 1 to 5*sqrt(blobs.len))
		if(!blobs.len) break
		var/obj/blob/B = pick(blobs)
		for (var/atom/A in B.loc) A.blob_act()
		B.Life()
		if(B && B.loc && istype(B.loc,/turf))
			B.loc:buildlinks()

/datum/game_mode/blob/proc/stage()
	// initial stage timing
	if(!next_stage)	next_stage = world.realtime + rand(200, 900)
	if(world.realtime < next_stage)	return
	...
	switch(stage)
		if(0)
			...
			world << "<font color='red'>Summary downloaded and printed out at all communications consoles.</font>"
			for(var/mob/silicon/ai/aiPlayer in world)
				if(!aiPlayer.client) continue
			...
			next_stage = world.realtime + 100*rand(30,60)

		if(1)
			world << "<font size = 3><B>Cent. Com. Update</B>: Biohazard Alert.</font>"
			world << "<font color='red'>Confirmed outbreak of level 5 biohazard aboard SS13.</font>"
			world << "<font color='red'>All personnel must contain the outbreak.</font>"
		...
		if(2)
			if(blobs.len > 500)
				world << "<font size = 3><B>Cent. Com. Update</B>: Biohazard Alert.</font>"
				world << "<font color='red'>Uncontrolled spread of the biohazard onboard the station.</font>"
				world << "<font color='red'>Cent. Com, has issued a directive 7-12 for Spacestation 13.</font>"
				world << "<font color='red'>Estimated time until directive implementation: 60 seconds.</font>"
				stage = 3
			next_stage = world.realtime + 600
		...
		if(3)
			...
			if(T) while(!isturf(T)) T = T.loc
			else  T = locate(45,45,1)
			...
			for(var/turf/U in block(sw, ne))
				var/zone = 4
				if((U.y <= T.y + max && U.y >= T.y - max && U.x <= T.x + max && U.x >= T.x - max))	zone = 3
				if((U.y <= T.y + med && U.y >= T.y - med && U.x <= T.x + med && U.x >= T.x - med))	zone = 2
				if((U.y <= T.y + min && U.y >= T.y - min && U.x <= T.x + min && U.x >= T.x - min))	zone = 1
				for(var/atom/A in U) A.ex_act(zone)
				U.ex_act(zone)
				U.buildlinks()
...
/datum/game_mode/blob/check_win()
	if(ticker.event == 4)
		...
		ticker.event = 5; ticker.check_win()
		return 1
	...
	if(!active)
		...
		ticker.event = 5
		world.log_game("Rebooting in 30s")
		sleep(300)
