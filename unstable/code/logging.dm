/world/proc/log_admin(text)
	...
	if(config.log_admin)
		world.log_file("[time2text(world.realtime)] - ADMIN: [text]")
		world.log << "ADMIN: [text]"

/world/proc/log_game(text)
	notify_admins("GAME: [text]")
	if(config.log_game)
		world.log_file("[time2text(world.realtime)] - GAME: [text]")
		world.log << "GAME: [text]"

/world/proc/log_bug(text)
	if(config.log_game)
		world.log_file("[time2text(world.realtime)] - BUG: [text]")
		world.log << "BUG: [text]"

/world/proc/log_vote(text)
	if(config.log_vote)
		world.log_file("[time2text(world.realtime)] - VOTE: [text]")
		world.log << "VOTE: [text]"

/world/proc/log_access(text)
	notify_admins("ACCESS: [text]")
	if(config.log_access)
		world.log_file("[time2text(world.realtime)] - ACCESS: [text]")
		world.log << "ACCESS: [text]"

/world/proc/log_say(text)
	if(config.log_say)
		world.log_file("[time2text(world.realtime)] - SAY: [text]")
		world.log << "SAY: [text]"

/world/proc/log_ooc(text)
	if(config.log_ooc)
		world.log_file("[time2text(world.realtime)] - OOC: [text]")
		world.log << "OOC: [text]"
