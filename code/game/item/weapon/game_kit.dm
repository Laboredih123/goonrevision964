...
/obj/item/weapon/game_kit/proc/update()
	...
	for (var/y = 1 to 8)
		...
		for (var/x = 1 to 8)
			...
			if (piece != "BB")
				dat += "<a href='?src=\ref[src];s_board=1 [counter]'><img src='[src.base_url]/board_[piece].png' width=32 height=32>"
			else
				dat += "&nbsp;"
			dat += "</td>"
	...
	for (var/piece in list("CB", "CR"))
		dat += "<a href='?src=\ref[src];s_piece=CB'><img src='[src.base_url]/board_[piece].png' width=32 height=32></a>"

	dat += "<HR><B>Chess pieces:</B><BR>"
	for (var/piece in list("WP", "WK", "WQ", "WI", "WN", "WR", "BP", "BK", "BQ", "BI", "BN", "BR"))
		dat += "<a href='?src=\ref[src];s_piece=[piece]'><img src='[src.base_url]/board_[piece].png' width=32 height=32></a>"
	src.data = dat
